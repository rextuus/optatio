...
class AppFixtures extends Fixture
{
    ...
    public function load(ObjectManager $manager): void
    {
        ...
        for ($i = 0; $i < 5; $i++) {
            ...
            $user->setPassword('$2y$13$tpF5lSTwDUasuUJmTm1AheHE1JQ5sy32HAVOJUMIr8DbliRe0KXWW');
    ...
    public function createSecretSantaEvent(){
        ...
        $event = $this->eventManager->initSecretSantaEvent($ssEventData, $user[0]);

        $data = new SecretSantaEventJoinData();
        $data->setFirstRound(true);
        $data->setSecondRound(true);
        $this->eventManager->addParticipantToSecretSantaEvent($user[0], $event, $data);

        $data = new SecretSantaEventJoinData();
        $data->setFirstRound(false);
        $data->setSecondRound(true);
        $this->eventManager->addParticipantToSecretSantaEvent($user[1], $event, $data);

        $data = new SecretSantaEventJoinData();
        $data->setFirstRound(true);
        $data->setSecondRound(false);
        $this->eventManager->addParticipantToSecretSantaEvent($user[2], $event, $data);
    }
}
